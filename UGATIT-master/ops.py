...
def adaptive_instance_layer_norm(x, gamma, beta, smoothing=True, scope='instance_layer_norm'):
    with tf.compat.v1.variable_scope(scope):
        ...
        ins_mean, ins_sigma = tf.nn.moments(
            x, axes=[1, 2])  # , keep_dims=True)
        x_ins = (x - ins_mean) / (tf.sqrt(ins_sigma + eps))

        ln_mean, ln_sigma = tf.nn.moments(
            x, axes=[1, 2, 3])  # , keep_dims=True)
        x_ln = (x - ln_mean) / (tf.sqrt(ln_sigma + eps))

        rho = tf.compat.v1.get_variable("rho", [ch], initializer=tf.constant_initializer(
            1.0), constraint=lambda x: tf.clip_by_value(x, clip_value_min=0.0, clip_value_max=1.0))
...
def instance_norm(x, scope='instance_norm'):
    instance_norma = tf.keras.layers.LayerNormalization(
        axis=-1,
        epsilon=1e-05,
        center=True, scale=True,
    )
    return instance_norma(x)


def layer_norm(x, scope='layer_norm'):
    layer_norma = tf.keras.layers.LayerNormalization(
        axis=-1,
        center=True, scale=True,
    )
...
def layer_instance_norm(x, scope='layer_instance_norm'):
    with tf.compat.v1.variable_scope(scope):
        ...
        rho = tf.compat.v1.get_variable("rho", [ch], initializer=tf.constant_initializer(
            0.0), constraint=lambda x: tf.clip_by_value(x, clip_value_min=0.0, clip_value_max=1.0))

        gamma = tf.compat.v1.get_variable(
            "gamma", [ch], initializer=tf.constant_initializer(1.0))
        beta = tf.compat.v1.get_variable(
            "beta", [ch], initializer=tf.constant_initializer(0.0))
...
def cam_loss(source, non_source):
    ...
    identity_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
        labels=tf.ones_like(source), logits=source))
    non_identity_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
        labels=tf.zeros_like(non_source), logits=non_source))
    ...
    for i in range(2):
        ...
        if loss_func == 'lsgan':
            real_loss = tf.reduce_mean(
                tf.math.squared_difference(real[i], 1.0))
            fake_loss = tf.reduce_mean(tf.square(fake[i]))

        if loss_func == 'gan' or loss_func == 'dragan':
            real_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
                labels=tf.ones_like(real[i]), logits=real[i]))
            fake_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
                labels=tf.zeros_like(fake[i]), logits=fake[i]))
    ...
    for i in range(2):
        ...
        if loss_func == 'lsgan':
            fake_loss = tf.reduce_mean(
                tf.math.squared_difference(fake[i], 1.0))
